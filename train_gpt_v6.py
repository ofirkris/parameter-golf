"""
train_gpt.py — Tap Mobile Optimal Combo for Parameter Golf.
Based on PR #338 (TTT) + PR #315 (XSA/EMA/PartialRoPE/LNScale/LateQAT) +
thwu1 (int5 MLP / int6 attn / BigramHash 10240 / magnitude pruning).
"""
...
class Hyperparameters:
    ...
    prune_global = bool(int(os.environ.get("PRUNE_GLOBAL", "1")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,vr_lambda",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.use_xsa = False
        # Value Residual: cache V from layer 0, mix into subsequent layers
        self.value_residual = value_residual
        if value_residual:
            self.vr_lambda = nn.Parameter(torch.tensor([0.5, 0.5], dtype=torch.float32))
        # Gated Attention: per-head sigmoid gate after attention output
    ...
    def forward(self, x: Tensor, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # Value Residual: save raw v for layer 0, mix v0 into later layers
        ...
        if self.gated_attention:
            ...
            y = y * gate.unsqueeze(-1)  # (bsz, seqlen, num_heads, 1) broadcast over head_dim
...
# -----------------------------
# INT5/INT6 MIXED QUANTIZATION (from thwu1 #1 + PR #315)
...
FP16_KEEP_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        f"tok_emb,blocks.{_num_layers_for_fp16 - 2}.attn.c_k",
    ).split(",")
    if pattern
)
...
def quantize_intN_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """Quantize to [-clip_range-1, clip_range] per-row."""
    t32 = t.float()
    if t32.ndim == 2:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            clip = 15 if cat == "mlp" else 31  # int5 for MLP, int6 for attention/bigram
...
def _ttt_run_epochs(base_model, params, optimizer, device, val_tokens, seq_len,
                    batch_seqs, epochs, rank, world_size, log_fn, phase_name, t0):
    """Run TTT epochs on val data with given optimizer and trainable params."""
    total_seqs = (val_tokens.numel() - 1) // seq_len
    ...
    for epoch in range(epochs):
        ...
        if log_fn:
            log_fn(f"ttt_{phase_name}_epoch:{epoch+1}/{epochs} loss:{epoch_loss_sum.item()/max(epoch_tokens.item(),1):.4f} time:{elapsed:.1f}s")


def ttt_adapt(args, base_model, device, val_tokens, rank=0, world_size=1, log_fn=None):
    """Two-Phase TTT: Phase 1 (norm-only recalibration) + Phase 2 (selective block adaptation)."""
...
def main() -> None:
    ...
    prune_frac = args.prune_fraction
    log0(f"pruning: fraction={prune_frac:.3f} global={args.prune_global}")
    with torch.no_grad():
        if args.prune_global:
            # Global pruning: find threshold across ALL large 2D params
            all_abs = []
            prune_params = []
            for name, param in base_model.named_parameters():
                if param.ndim == 2 and param.numel() > 65536:
                    all_abs.append(param.abs().float().flatten())
                    prune_params.append(param)
            if all_abs:
                all_abs_cat = torch.cat(all_abs)
                threshold = torch.quantile(all_abs_cat, prune_frac)
                total_pruned = 0
                total_params = 0
                ...
                log0(f"pruning: zeroed {total_pruned}/{total_params} params ({100*total_pruned/total_params:.1f}%)")
                del all_abs_cat, all_abs
        else:
            # Per-parameter pruning (original behavior)
            for name, param in base_model.named_parameters():
                if param.ndim == 2 and param.numel() > 65536:
    ...
    if args.ttt_enabled:
        if distributed:
            dist.barrier()
        log0(f"ttt:start phase1_epochs={args.ttt_phase1_epochs} phase1_lr={args.ttt_phase1_lr} phase2_epochs={args.ttt_epochs} phase2_lr={args.ttt_lr} unfreeze_blocks={args.ttt_phase2_unfreeze_blocks}")
